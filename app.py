...
# 1. הגדרות דף ועיצוב מקסימלי
st.set_page_config(page_title="והשיב - CRM ניהול תורמים", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown("""
    ...
    /* סרגל עליון כחול נדרים */
    .top-nav { background-color: #3b566e; color: white; padding: 10px 20px; display: flex; justify-content: space-between; border-bottom: 4px solid #00b5ad; }
    
    /* סרגל אייקונים עגולים */
    .icon-bar { background-color: #5591ab; padding: 15px; display: flex; justify-content: center; gap: 25px; margin-bottom: 20px; border-radius: 0 0 15px 15px; }
    .icon-item { color: white; text-align: center; font-size: 12px; }
    .icon-circle { background-color: white; color: #5591ab; width: 45px; height: 45px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 5px; font-size: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.2); }
    .active-icon { background-color: #00b5ad !important; color: white !important; }

    /* טבלה טורקיזית */
    .nedarim-header { background-color: #00b5ad; color: white; padding: 12px; font-weight: bold; display: flex; border-radius: 8px 8px 0 0; margin-top: 20px; }
    .table-row { background-color: white; border-bottom: 1px solid #eee; padding: 10px; display: flex; align-items: center; }
    .table-row:hover { background-color: #f9f9f9; }

    /* כרטיס תורם (ה-Popup) */
    .donor-modal { background-color: white; border-radius: 15px; padding: 30px; box-shadow: 0 10px 40px rgba(0,0,0,0.2); border-right: 10px solid #00b5ad; }
    .field-box { background: #f8fbfc; border: 1px solid #e1e8eb; padding: 8px 12px; border-radius: 5px; margin-bottom: 10px; min-height: 40px; }
    .field-label { font-size: 13px; color: #666; margin-bottom: 2px; }
    </style>
...
# --- סרגל עליון ואייקונים ---
st.markdown("""
    <div class="top-nav">
        <div style="font-size:18px; font-weight:bold;">והשיב - ארגון להורים לנוער מתמודד</div>
        <div style="font-size:14px;">שלום, מנהל המערכת | <a href="#" style="color:white;">יציאה</a></div>
    </div>
    <div class="icon-bar">
...
if password == "dudi330582008":
    ...
    if st.session_state.page == 'list':
        ...
        st.markdown("""
            <div class="nedarim-header">
                <div style="flex:1">מכלול</div><div style="flex:3">שם התורם</div>
                <div style="flex:3">כתובת</div><div style="flex:2">טלפון</div><div style="flex:1">ערוך</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    # --- תצוגת כרטיס תורם (ה-Popup) ---
        ...
        # כפתור אשראי עם PIN (הפתרון של ג'ימיני)
        st.write("")
        with st.popover("💳 צפייה בפרטי אשראי"):
            pin = st.text_input("הזן קוד PIN לאבטחה:", type="password")
            if pin == "1234": # שנה כאן ל-PIN שאתה רוצה
        ...
        # פילטר חגים
        filter_h = st.radio("סנן לפי:", ["הכל", "פסח", "פורים", "ר''ה", "חנוכה"], horizontal=True)
        ...
        if history:
            ...
            st.markdown(f"<h3 style='color:#00b5ad;'>סך הכל מכלול: ₪{donor['מכלול']:,.0f}</h3>", unsafe_allow_html=True)
        
        st.markdown('</div>', unsafe_allow_html=True)

else:
    st.info("אנא הכנס סיסמת מערכת בתפריט הצד")
